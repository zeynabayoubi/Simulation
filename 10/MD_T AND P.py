...
def mean (T , P , n_s , tau , h):
        temp = np.zeros(len(T) - int(tau/(h*n_s)))
        pres = np.zeros(len(P) - int(tau/(h*n_s)))
        temp[:] = T[int(tau/(h*n_s)):]
        pres[:] = P[int(tau/(h*n_s)):]
        return np.mean(temp) , np.mean(pres)
...
h = 0.01
r_c = 2.5
v_max = np.arange(0.5 , 2 , 0.1)
n_s = 10


Energy = np.zeros(len(v_max))
temperature = np.zeros(len(v_max))
pressure = np.zeros(len(v_max)) 

for k in range (len(v_max)):
        print ("k =" , k)
        X , Y = place_atoms_left_side_regularly(N , L)
        Vx , Vy = initial_velocities(L , N , v_max[k])
        a_x , a_y  , U , P = accel(X , Y , L , N , r_c)
        ...
        for i in range (len(t)):
            ...
            if i%n_s == 0:
                ...
                n_L.append(n_left(X , L , N))
                print(i) 

        Kinetic_Energy = np.array(Kinetic_Energy)
        Potential_Energy = np.array(Potential_Energy  )
        Total_Energy = Kinetic_Energy + Potential_Energy
        Energy[k] = Total_Energy[500]

        Temperature = np.array(Temperature)
        Pressure = np.array(Pressure)
        temperature[k] , pressure[k] = mean (Temperature , Pressure , n_s , 10 , h)


m , b = np.polyfit(Energy , temperature , 1)
x = np.arange(Energy[0] , Energy[len(v_max)-1] , 0.1)
y = m*x + b
plt.plot(x,y)
plt.scatter (Energy , temperature)
...
m , b = np.polyfit(Energy , pressure , 1)
x = np.arange(Energy[0] , Energy[len(v_max)-1] , 0.1)
y = m*x + b
plt.plot(x,y)
...
m , b = np.polyfit(temperature , pressure , 1)
x = np.arange(temperature[0] , temperature[len(v_max)-1] , 0.1)
y = m*x + b
plt.plot(x,y)
